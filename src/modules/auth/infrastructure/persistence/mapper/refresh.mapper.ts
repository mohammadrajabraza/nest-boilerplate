...
import { RefreshResponseDto, RefreshTokenResponseDto } from '@/modules/auth/dtos/response/refresh-token.dto';
import { AuthTokenResponseDto, TokenPayload } from '@/modules/token/dtos/auth-token.dto';
import { HttpStatus } from '@nestjs/common';

export class RefreshTokenMapper {
  static toDomain(tokens: Record<'access' | 'refresh', { token: string; expiresIn: number }>) {
    ...
    return new RefreshTokenResponseDto(data, successMessage.AUTH.REFRESH_TOKEN, HttpStatus.OK);
  }
